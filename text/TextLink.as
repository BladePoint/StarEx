...
package starEx.text {
	...
	import starEx.display.ButtonEx;
	...
	public class TextLink extends ButtonEx {
		...
		private function resizeHitbox():void {
			...
			for (var i:uint=0; i<l; i++) {
				...
				const hitbox_AQ:ApertureQuad = getHitbox(line_AQ.localW,aboveLineH+line_AQ.localH);
		...
		public function setColor(linkStatus:String):void {
			...
			for (var i:uint=0; i<l; i++) {
				...
				charLocation.updateColor(topLeft,topRight,bottomLeft,bottomRight);
			}
		}
		...
		private function updateOutlineColor(outlineColor:uint):void {
			...
			for (var i:uint=0; i<l; i++) {
				const charLocation:CharLocation = charLocationV[i];
				charLocation.updateOutlineColor(outlineColor);
			}
		}
