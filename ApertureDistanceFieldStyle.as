...
package starEx.styles {
	...
	import starEx.styles.DistanceFieldEffect;
	/* Much of this code is appropriated from starling.styles.DistanceFieldStyle. */
	public class ApertureDistanceFieldStyle extends MeshStyle implements IAperture {
		...
		static public function setStyleWidth(style:MeshStyle,width:Number):void {
			if (style is DistanceFieldStyle || style is ApertureDistanceFieldStyle) {
				const styleObject:Object = style as Object;
				const threshold:Number = styleObject.threshold;
				styleObject.outerThreshold = MathUtil.clamp(threshold-width,0,threshold);
			}
		}
		static public function getStyleWidth(style:MeshStyle):Number {
			var width:Number = 0;
			if (style is DistanceFieldStyle || style is ApertureDistanceFieldStyle) {
				const styleObject:Object = style as Object;
				width = styleObject.threshold - styleObject.outerThreshold;
			}
			return width;
		}
		...
		public function setupOutline(width:Number=.25,color:uint=0x000000,alpha:Number=1):void {
			_mode = MODE_OUTLINE;
